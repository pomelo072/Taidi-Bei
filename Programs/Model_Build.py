...
def Decision():
    ...
    #模型构建
    Model = DecisionTreeClassifier(max_depth=25,random_state=8,splitter='random',min_samples_split=3,min_samples_leaf=1,)
    #模型训练
...
def Voting():
    #读取文件
    data1 = pd.read_csv('pre_test_data.csv',encoding = 'GBK')
    dat = data1.loc[data1["年份（年末）"]!=7]
    data = dat.loc[dat["年份（年末）"]!=8]

    print('------------投票------------')
    #处理数据不均衡问题
    ros = RandomOverSampler(random_state=18,sampling_strategy=0.56)
    X_resampled, y_resampled = ros.fit_sample(data.iloc[:, 3:15], data['是否高转送'])
    #拆分专家样本集
    X_train, X_test, y_train, y_test = train_test_split( X_resampled, y_resampled,test_size=0.001)
    #模型构建
    voting_clf = VotingClassifier(estimators=[
        ('log_pre',LogisticRegression(max_iter=5000)),
        ('KN_pre',KNeighborsClassifier(n_neighbors=4)),
        ('dt_pre',DecisionTreeClassifier(max_depth=25,splitter='random',min_samples_split=3,min_samples_leaf=1,))
    ],voting='hard')
    #模型训练
    ...
    dataframe.to_csv("nine_pre.csv", index=False, encoding='GBK')
    print("第八年预测数据保存成功！")
#模型调用并输出预测结果到csv文件
def Model_load():
    #加载训练好的模型
